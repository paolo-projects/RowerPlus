...
class UsbSerialConnectionManager(private val context: Context, private val listener: Listener) {
    ...
    fun connect(usbDevice: UsbDevice?) {
        ...
        val probeTable = ProbeTable().apply {
            addProduct(1155, 14155, CdcAcmSerialDriver::class.java)
        }
        val serialProber = UsbSerialProber(probeTable)
        val availableDrivers = serialProber.findAllDrivers(usbManager)
